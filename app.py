...
# 1. Streamlit Page Configuration & Professional Executive Styling CSS
st.set_page_config(
...
st.markdown("""
    <style>
        ...
        /* Apply Professional Clean Font Globally */
        html, body, [class*="css"] {
            font-family: 'Inter', sans-serif !important;
            font-size: 16px !important;
        }

        /* Deep Matte Executive Black Theme */
        .stApp {
        ...
        /* Completely HIDE instruction popups */
        ...
        /* Sleek dark inputs with subtle border */
...
# 2. Centered Customer Login Screen (Before Login) positioned down with professional fonts
if not st.session_state.logged_in_user:
    ...
    st.markdown("""
        <div class="login-title">
            <h1>HM Mobiles Thiruverkadu</h1>
            <p>Premium Mobile Accessories & Service &nbsp;|&nbsp; 📞 9840450113</p>
        </div>
    """, unsafe_allow_html=True)
        ...
        with st.container(height=2400, border=True):
            ...
            if filtered_items:
                ...
                for idx, prod in enumerate(current_page_items):
                        ...
                        for i in range(3):
                            with img_cols_1[i]:
                                if i < len(valid_paths):
                                    ...
                                    st.markdown("<p style='text-align: center; color: #4b5563; font-size: 13px;'>No Img</p>", unsafe_allow_html=True)
                    ...
                    with p_div1_col:
                        st.markdown("<div style='border-left: 1px solid #374151; height: 350px; margin-top: 1px;'></div>", unsafe_allow_html=True)
                        ...
                        with q_minus:
                            if st.button("-", key=f"minus_{current_cat}_{global_idx}", use_container_width=True):
                                if st.session_state.quantities[qty_key] > 1:
                                    ...
                                    st.rerun()
                        with q_display:
                            st.markdown(f"<div style='text-align: center; padding-top: 6px; font-weight: 600; color: #ffffff;'>{st.session_state.quantities[qty_key]}</div>", unsafe_allow_html=True)
                        with q_plus:
                    ...
                    st.markdown("<hr style='margin-top: 4px; margin-bottom: 4px; border: none; border-top: 1px solid #374151;'>", unsafe_allow_html=True)
                    ...
                    with pg_prev:
                        if st.button("⬅ Prev", use_container_width=True):
                            if st.session_state.product_page > 0:
                                ...
                                st.rerun()
                    with pg_info:
                        st.markdown(f"<p style='text-align: center; margin-top: 2px; color: #9ca3af;'>Page {st.session_state.product_page + 1} of {total_pages}</p>", unsafe_allow_html=True)
                    with pg_next:
